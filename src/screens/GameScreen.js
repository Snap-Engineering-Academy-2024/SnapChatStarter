...
export default function TopicsScreen() {
    ...
    const executeCommands = () => {
        ...
        for(let i = 0; i < commandList.length; i++) {
            transformSprite(commandList[i]);

            // Check if indices out of bounds or illegal space entered
            if (currentX < 0 || currentX >= gameMatrix[0].length
                || currentY < 0 || currentY >= gameMatrix.length
                || gameMatrix[currentY][currentX] != 1 || invalidBoom) {
                    console.log("Illegal move! Game ended.");
                    break;
                }
        }

        // Check if final coordinates match destination coordinates
    ...
    const boomBoulder = () => {
        switch(currentFacing) {
            case 0:
                if (currentY + 1 >= gameMatrix.length || gameMatrix[currentY + 1][currentX] != 2) {
                    invalidBoom = true;
                }
                else {
                ...
                break;
            case 1:
                if (currentX + 1 >= gameMatrix[0].length || gameMatrix[currentY][currentX + 1] != 2) {
                    invalidBoom = true;
                }
                else {
